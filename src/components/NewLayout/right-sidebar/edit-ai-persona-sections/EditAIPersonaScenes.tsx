...
const EditAIPersonaScenes: React.FC<EditAIPersonaScenesProps> = ({ onCapsuleClick }) => {
  ...
  return (
    <div className="flex space-x-4 overflow-x-auto pb-6 scrollbar-hide snap-x snap-mandatory scroll-px-6">
      {sceneCards.map(card => {
        const ref = (capsuleRefs.current[card.text] = capsuleRefs.current[card.text] || null);
        return (
          <div
            key={card.text}
            ref={(el) => { capsuleRefs.current[card.text] = el; }}
            onClick={() => {
              dispatch(setCurrentScene(card));
              onCapsuleClick(card, capsuleRefs.current[card.text]);
            }}
